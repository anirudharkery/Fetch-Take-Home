...
#             print(f"Endpoint {url} is {status}. Response time: {response_time:.2f}ms, Status code: {response.status_code}")
#             return is_up
#         except requests.RequestException as e:
#             print(f"Endpoint {url} is DOWN. Error: {str(e)}")
#             return False
...
#         try:
#             while True:
#                 print(f"\nStarting health check cycle at {time.strftime('%Y-%m-%d %H:%M:%S')}")
#                 self.run_health_checks()
#                 self.log_availability()
#                 print("Waiting 15 seconds until next check...")
#                 time.sleep(15)
